...
function App() {
  ...
  const onSubmit = e =>{
    ...
    axios.get('http://kun.localhost:8000/sanctum/csrf-cookie').then(response => {
        axios.post('http://kun.localhost:8000/api/login',state).then(res=>{
          console.log(res)
        })
    });
        ...
        <div className="rounded-md shadow-sm">
          <div>
            <input
                ...
                aria-label="Username" name="handle" type="string" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" placeholder="Username"/>
          </div>
          <div className="-mt-px">
            <input
                onChange={handleChange}
